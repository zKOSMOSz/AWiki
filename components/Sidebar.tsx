...
const NavItem: React.FC<{ item: WikiTreeItem; activePageId: string | null; onSelectPage: (id: string) => void; level: number }> = ({ item, activePageId, onSelectPage, level }) => {
  const isCollapsible = item.type === 'section' && item.id === 'alkogol';
    ...
    return (
      <div>
        <button
          ...
          className={`${commonClasses} justify-between ${isActive ? activeClasses : inactiveClasses}`}
        >
  ...
  return (
    <div className="h-full flex flex-col text-sm">
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
            {wikiTree.map((item) => {
              if (item.type === 'section' && !item.path && item.id !== 'alkogol') {
                 return (
